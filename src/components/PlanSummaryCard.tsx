...
export default function PlanSummaryCard({ contractTime, holdTime, relaxTime, rounds, sets, restBetweenSets, voice, onClick }: Props) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full space-y-2 rounded-xl border border-warm-200/[0.06] bg-warm-200/[0.04] p-4 text-left transition-colors hover:bg-warm-200/[0.07]"
      aria-label="编辑当前训练计划"
    >
      <h2 className="text-sm font-semibold text-warm-200">基础训练</h2>
      ...
      <div className="flex items-center gap-3 pt-1 text-[10px] text-warm-400">
        <span className={voice.enabled && voice.mode !== 'off' ? 'text-emerald-400' : 'text-warm-500'}>
          {voice.enabled && voice.mode !== 'off' ? '语音教练已开启' : '语音已关闭'}
        </span>
        <span className={voice.hapticsEnabled ? 'text-emerald-400' : 'text-warm-500'}>
