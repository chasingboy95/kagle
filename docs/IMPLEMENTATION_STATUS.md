...
| Accessibility | Partial | Recovery takes priority over onboarding; dialogs have labels/descriptions, focus traps and restoration, and explicit Escape behavior. Shared focus rings, reduced-motion transition suppression, 44px core targets, 200% text coverage, and Playwright axe serious/critical blocking are automated (#93). Real-device screen-reader verification remains outstanding. |
| Mobile UX / Navigation | Partial | The staged roadmap is implemented through #93. The primary navigation uses a compact 56px tab bar with consistent SVG icons, is fixed to the visual viewport bottom with a matching in-flow spacer, remains the sole idle safe-area owner, and shares one opaque surface with the root canvas and action dock so iOS cannot expose a contrasting Home Indicator strip (#103, #105, #109). Dedicated records/settings pages, explicit draft submission, 12px supporting text, 44px core controls, narrow-screen statistics, and shared bottom-sheet behavior remain in place. iOS PWA revalidation plus iOS Safari and Android Chrome final device acceptance remain outstanding. |
...
| PWA / GitHub Pages | Partial | Manifest, service worker, subpath, standard PNG icons, a versioned Apple Touch Icon with a conventional root fallback, update activation, offline caching, and voice asset cache are covered by automated tests. The root document is fixed to the standalone viewport while `.app-shell` exclusively owns page scrolling. Bottom safe-area ownership belongs to the fixed primary navigation while idle and transfers to the action dock only when navigation is hidden during training; both surfaces match the root canvas color (#82, #103, #105, #109). The app checks for updates on foreground entry and automatically activates a waiting worker only in idle/feedback states; active training remains protected. Existing iOS Web Clips must be deleted and re-added for icon artwork changes, and iPhone standalone-mode revalidation remains outstanding. |
...
| Training History | Complete | A dedicated page replaces the training home while open and provides a page-level title, back navigation with focus restoration, natural document scrolling, and real all/completed/stopped list filtering with filter-specific empty states. List and timezone-aware monthly calendar views expose real records. The calendar distinguishes completed and stopped sessions, opens daily records in the shared detail flow, and reports completed sessions, completed days, duration, and longest streak. Detail supports objective fields and confirmed deletion (#89). |
...
- Primary navigation selection, settings grouping, plan-card entry, browser back between pages, and navigation removal during an active session (#91).
- Native reminder time selection, 44px weekday targets, settings detail navigation, and dedicated data-management entry on a 320px viewport (#92).
