...
| Mobile UX / Navigation | Partial | The staged roadmap is implemented through #93. The primary navigation uses a compact 56px translucent tab bar with consistent SVG icons, is fixed to the visual viewport bottom with a matching in-flow spacer, and remains the sole idle safe-area owner (#103, #105). Dedicated records/settings pages, explicit draft submission, 12px supporting text, 44px core controls, narrow-screen statistics, and shared bottom-sheet behavior remain in place. iOS PWA revalidation plus iOS Safari and Android Chrome final device acceptance remain outstanding. |
...
| PWA / GitHub Pages | Partial | Manifest, service worker, subpath, standard PNG icons, apple-touch-icon, update activation, offline caching, and voice asset cache are covered by automated tests. The root document is fixed to the standalone viewport while `.app-shell` exclusively owns page scrolling. Bottom safe-area ownership belongs to the fixed primary navigation while idle and transfers to the action dock only when navigation is hidden during training (#82, #103, #105). The app checks for updates on foreground entry and automatically activates a waiting worker only in idle/feedback states; active training remains protected. iPhone standalone-mode revalidation remains outstanding. |
