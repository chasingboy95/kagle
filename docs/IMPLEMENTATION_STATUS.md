...
| Mobile UX / Navigation | Partial | The staged roadmap is implemented through #93. The primary navigation now uses a compact 56px translucent tab bar with consistent SVG icons and one bottom safe-area owner; the idle action dock and records content no longer duplicate the Home Indicator inset (#103). Dedicated records/settings pages, explicit draft submission, 12px supporting text, 44px core controls, narrow-screen statistics, and shared bottom-sheet behavior remain in place. iOS PWA, iOS Safari, and Android Chrome final device acceptance remains outstanding. |
...
| PWA / GitHub Pages | Partial | Manifest, service worker, subpath, standard PNG icons, apple-touch-icon, version update notification, offline caching, and voice asset cache are covered by Playwright E2E tests. The root document is fixed to the standalone viewport while `.app-shell` exclusively owns page scrolling. Bottom safe-area ownership belongs to the primary navigation while idle and transfers to the action dock only when navigation is hidden during training (#82, #103). Automated coverage verifies no root overflow at 320×568; iPhone standalone-mode safe-area revalidation remains outstanding. Service worker activation is user-triggered and blocked during a live training session. |
