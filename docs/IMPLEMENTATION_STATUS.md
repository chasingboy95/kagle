...
| PWA / GitHub Pages | Complete | Manifest, service worker, subpath, safe-area, standard PNG icons, apple-touch-icon, version update notification, offline caching, and voice asset cache verified by Playwright E2E tests (e2e/pwa.spec.ts). Real-device QA remains outstanding. Service worker no longer auto-activates on install: the new worker stays in the "waiting" state, and `showUpdatePrompt` only requests activation (`SKIP_WAITING`) when the user clicks "立即更新" and no live training session (running/paused snapshot) exists; the feedback view is treated as safe to update. |
