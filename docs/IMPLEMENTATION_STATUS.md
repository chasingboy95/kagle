...
| Training History | Complete | Keeps the newest 500 records with deterministic eviction. The list opens an objective record-detail view with timing, rhythm, repetitions, status, preset/custom identification, back navigation, and confirmed deletion. Per-record validation isolates corrupt records; persistence failures retain current UI state and show a warning. |
