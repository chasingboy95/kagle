...
| Voice Settings Panel | Complete | The settings drawer edits a local draft, previews that draft without persisting it, and commits only through “应用设置”; dirty close, cancel, backdrop, and Escape paths require an explicit discard choice. UI exposes common controls first and includes an optional pre-training self-check with the current mode/countdown/volume, a short preview, “我能听到” confirmation, explicit silent choice, and non-technical failure guidance (#90). |
| Training Plan Editor | Complete | The drawer edits a local plan draft and commits only through “应用此计划”. Dirty close, cancel, backdrop, and Escape paths require an explicit discard choice; presets remain single-select and the drawer exposes controls directly instead of nesting them behind another collapsed summary (#90). |
